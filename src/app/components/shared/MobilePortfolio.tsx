...
export default function MobilePortfolio() {
  ...
  return (
    <section className="relative py-8 px-4 bg-gradient-to-br from-gray-900 via-gray-900 to-gray-900 min-h-screen overflow-x-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <div className="inline-flex p-4">
            <motion.div 
              ...
              className="flex items-center gap-1 text-xs text-orange-400 font-mono px-2 py-2 bg-transparent rounded-md border border-orange-400"
            >
        ...
        >
          ...
          <motion.div 
            className="absolute left-0 top-1/2 transform -translate-y-1/2 w-[80%] h-[360px] bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-600 z-0"
            style={{ x: '-80%' }}
          ...
          <motion.div 
            className="absolute right-0 top-1/2 transform -translate-y-1/2 w-[80%] h-[360px] bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-600 z-0"
            style={{ x: '80%' }}
            initial={{ opacity: 0.3, scale: 0.8 }}
            animate={{ opacity: 0.3, scale: 0.8 }}
            transition={{ duration: 0.3 }}
          ...
          <div className="relative min-h-[400px]">
            <AnimatePresence custom={direction} initial={false}>
              <motion.div
                ...
                className="absolute inset-0 bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 w-full"
                drag="x"
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {filteredProjects[currentIndex].tags.map(tag => (
                      <span 
                        key={tag}
                        className="text-[0.7rem] font-mono bg-gray-900 text-blue-500 px-1.5 py-0.5 rounded"
                      >
                        {tag}
                      </span>
                    ))}
                    ...
                    {filteredProjects[currentIndex].link && (
                      <div className="mt-3 pt-3 border-t border-gray-700">
                        <button
                          ...
                          className="border border-blue-500/40 text-white p-2 rounded-lg text-center bg-blue-600/30 w-full text-xs font-mono hover:bg-blue-700/40 transition"
                        >
                          VIEW PROJECT
                        </button>
                      </div>
        ...
        {/* Replace the Tech Stack Overview section with this: */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5 }}
          className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-gray-700 mb-8"
        >
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-sm font-mono text-blue-500 flex items-center gap-2">
              <BrainCircuit size={16} />
              GLITCH_TERMINAL
            </h3>
            <button
              onClick={() => setActiveFilter('All')}
              className="text-xs font-mono px-2 py-1 bg-gray-900/50 border border-gray-600 rounded hover:bg-gray-800 transition text-gray-300"
            >
              [DEFAULT_STATE]
            </button>
          </div>

          <div className="bg-gray-900/70 p-3 rounded-lg border border-blue-500 font-mono text-xs mb-3">
            <div className="text-blue-500 mb-2">$ tech_stack --analyze</div>
            <div className="text-gray-400">&gt; Scanning {allTags.length} technologies...</div>
          </div>

          <div 
            className="bg-gray-900/70 p-3 rounded border-lg border-gray-700 cursor-pointer hover:bg-gray-800/50 transition mb-3 min-h-20 flex flex-col justify-center"
            onClick={() => {
          ...
          >
            <div className="flex justify-between text-gray-500 text-xs mb-2">
              ...
              <span className="text-blue-500">TAP_TO_DECODE</span>
            </div>
                ...
                return (
                  <div className="space-y-1">
                    <div className="text-purple-500 font-medium">
                      {maskedTech}
                      <span className="text-gray-500 text-xs block">// tech_stack</span>
                    </div>
                ...
                return (
                  <div className="space-y-1">
                    <div className="text-green-400 font-medium">
                      $ {maskedCode}
                      <span className="text-gray-500 text-xs block">// terminal_command</span>
                    </div>
                    <div className="text-gray-500 text-xs mt-1">
        ...
        {filteredProjects.length === 0 && (
          <div className="text-center py-12">
            <icons.Code size={32} className="mx-auto text-gray-600 mb-3" />
            <h3 className="text-lg text-gray-400 font-mono">{portfolioText.noProjects.title}</h3>
            <p className="text-gray-500 mt-1 mb-4 text-sm">{portfolioText.noProjects.message}</p>
            <button
              onClick={() => setActiveFilter('All')}
