...
export default function DesktopExperience() {
  ...
  return (
    <div className="h-full bg-gradient-to-br from-gray-900 to-gray-800 p-4 sm:p-8 overflow-auto">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Experience Timeline */}
        <div className="mb-16">
          <h2 className="text-xl font-mono text-gray-200 mb-6 flex items-center gap-2">
            <Wrench className="text-blue-500" size={20} />
            CAREER_TIMELINE
          </h2>
          
          <div className="relative">
            <div className="absolute left-5 top-0 h-full w-0.5 bg-blue-900"></div>
            ...
            {experiences.map((exp, index) => (
              ...
              >
                <div className="absolute left-5 top-1/3 transform -translate-x-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-blue-500 border-4 border-blue-900 z-10 animate-pulse"></div>
                
                <div 
                  className="bg-gray-800 backdrop-blur-sm rounded-xl p-5 shadow-sm border border-gray-700 hover:shadow-md hover:shadow-blue-500/10 hover:border-blue-500 transition-all duration-300 cursor-pointer"
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <span className="text-xs font-mono text-gray-400 bg-gray-900 px-2 py-1 rounded inline-block mb-2">
                        {exp.period}
                      </span>
                      <p className="text-s text-gray-300 mb-3">{exp.description}</p>
                      <div className="flex flex-wrap gap-2 mb-3">
                        {exp.tech.map((tech, i) => (
                          <span key={i} className="text-xs font-mono bg-gray-900 text-blue-500 px-2 py-1 rounded">
                            {tech}
                          </span>
                        ))}
                      </div>
        ...
        <div className="mb-8">
          ...
          <div className="relative">
            <button 
              ...
              className={`absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-gray-800/80 hover:bg-gray-700/90 p-2 rounded-full border border-gray-700 text-white ml-2 transition-opacity ${
              ...
              {projects.map((project, index) => {
                ...
                return (
                  <motion.div
                    key={index}
                    whileHover={{ y: -5 }}
                    className="flex-shrink-0 w-64 bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl overflow-hidden shadow-lg hover:shadow-blue-500/10 transition-all cursor-pointer relative"
                    style={{ scrollSnapAlign: 'start' }}
                    onClick={(e) => handleProjectClick(index, e)}
                  >
                    {project.link && (
                      <div className="absolute top-2 right-2 bg-gray-900/80 rounded-full p-1 z-10">
                    ...
                    <div className="p-4">
                      <h3 className="font-mono text-white font-medium mb-1">{project.title}</h3>
                      <p className="text-xs text-gray-300 text-sm mb-3">{project.description}</p>
                      <div className="flex flex-wrap gap-1">
                        {project.tags.map((tag, i) => (
                          <span key={i} className="text-xs font-mono bg-gray-900/70 text-blue-500 px-2 py-1 rounded">
                            {tag}
                          </span>
                        ))}
                      </div>
                      ...
                      {!project.link && (
                        ...
                        >
                          <div className="pt-3 mt-3 border-t border-gray-700">
                            <div className="text-xs font-mono border border-blue-500/40 text-white p-2 rounded-lg text-center bg-blue-600/30">
            ...
            <button 
              ...
              className={`absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-gray-800/80 hover:bg-gray-700/90 p-2 rounded-full border border-gray-700 text-white mr-2 transition-opacity ${
